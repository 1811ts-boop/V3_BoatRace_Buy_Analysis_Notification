...
def prepare_ai_models(service):
    logger.info("☁️ V12専用 AIモデル・マスターデータ・ポートフォリオをダウンロードします...")
    os.makedirs("Models_Stage1_V12", exist_ok=True)
    os.makedirs("Models_Stage2_V12", exist_ok=True)
    ...
    # 💡 統合されたV12のポートフォリオCSVをダウンロード
    PORTFOLIO_V12_CSV = "V12_Portfolio_OOS_Detailed_Results.csv"
    download_latest_file_by_name(service, PORTFOLIO_V12_CSV)
    
    for pid in PROJECT_IDS:
        download_latest_file_by_name(service, f"LGBM_Stage1_V12_{pid}.pkl", "Models_Stage1_V12")
        download_latest_file_by_name(service, f"LGBM_V12_Model1_1st_{pid}.pkl", "Models_Stage2_V12")
        download_latest_file_by_name(service, f"LGBM_V12_Model2_2nd_{pid}.pkl", "Models_Stage2_V12")
...
def transform_for_v12_inference(df_raw, df_tide, dict_motor, dict_boat):
    fs1, fs2 = [], []
    error_count = 0 
...
# =============================================================================
# 6. V12 ハイブリッド推論 ＆ LINE通知 (S/A/Bランク 資金プッシュ完全自動化版)
# =============================================================================
...
def run_v12_inference_and_notify(df_s1, df_s2):
    # 💡 [追加] LINE通知用のお宝条件リスト
    V12_TREASURES = [
        {'type': '2fuku', 'prob_cat': 0.35, 'places': ["蒲郡", "徳山", "唐津", "尼崎"]},
        {'type': '2tan',  'prob_cat': 0.15, 'places': ["宮島", "尼崎"]},
        {'type': '2fuku', 'prob_cat': 0.15, 'places': ["唐津", "尼崎", "浜名湖"]},
        {'type': '2tan',  'prob_cat': 0.10, 'places': ["びわこ", "尼崎", "津", "唐津", "浜名湖"]},
        {'type': '2fuku', 'prob_cat': 0.25, 'places': ["江戸川", "多摩川"]},
        {'type': '2fuku', 'prob_cat': 0.30, 'places': ["大村", "徳山", "尼崎"]},
    ]

    current_month = TODAY_OBJ.month
    
    # 💡 読み込むポートフォリオをV12の統合版に変更
    PORTFOLIO_V12_CSV = "V12_Portfolio_OOS_Detailed_Results.csv"
    if not os.path.exists(PORTFOLIO_V12_CSV):
        send_line_broadcast("❌ V12ポートフォリオファイルが見つかりません。稼働を停止します。")
        return
        
    df_port = pd.read_csv(PORTFOLIO_V12_CSV)
    ...
    # 💡 2単と2複のフィルターフラグと倍率を各行で計算
    df_port['Plus_Ratio_2t'] = df_port['OOS_プラス年数(25~26年)'].apply(get_plus_ratio)
    df_port['Plus_Ratio_2f'] = df_port['OOS_2連複_プラス年数(25~26年)'].apply(get_plus_ratio)

    df_port['Is_Golden_2t'] = (df_port['OOS(未知)_統合レース数'] >= 15) & (df_port['OOS(未知)_統合ROI'] >= 110) & (df_port['Plus_Ratio_2t'] >= 0.6)
    df_port['Is_Golden_2f'] = (df_port['OOS(未知)_統合レース数'] >= 15) & (df_port['OOS(未知)_統合2連複_ROI'] >= 110) & (df_port['Plus_Ratio_2f'] >= 0.6)

    # 総合力スコアの計算
    df_port['Score_2t'] = np.where(df_port['Is_Golden_2t'], (df_port['OOS(未知)_統合ROI'] - 100) / 100 * df_port['OOS(未知)_統合レース数'], -1)
    df_port['Score_2f'] = np.where(df_port['Is_Golden_2f'], (df_port['OOS(未知)_統合2連複_ROI'] - 100) / 100 * df_port['OOS(未知)_統合レース数'], -1)
    ...
    t_S_2t, t_A_2t = calc_thresholds(df_port['Score_2t'])
    t_S_2f, t_A_2f = calc_thresholds(df_port['Score_2f'])
    ...
    df_port['Multi_2t'] = df_port['Score_2t'].apply(lambda x: get_tier_multi(x, t_S_2t, t_A_2t))
    df_port['Multi_2f'] = df_port['Score_2f'].apply(lambda x: get_tier_multi(x, t_S_2f, t_A_2f))

    valid_conditions_2t = {(r['Month'], r['Project_ID'], r['場名'], r['Rough_Category']): r['Multi_2t'] for _, r in df_port.iterrows() if r['Multi_2t'] > 0}
    valid_conditions_2f = {(r['Month'], r['Project_ID'], r['場名'], r['Rough_Category']): r['Multi_2f'] for _, r in df_port.iterrows() if r['Multi_2f'] > 0}
    ...
    for pid in PROJECT_IDS:
        ...
        try:
            ...
            # 💡 Stage 2に渡すデータに V12の新特徴量 を含める
            race_info_s1 = ds1[['Race_ID', 'PlaceID', 'Scheduled_Time', 'Stage1_Rough_Prob', 'F_Program_Distortion_Index', 'F_Dash_ST_Threat']].drop_duplicates('Race_ID')
            ...
            # --- ⚡ Stage 2 準備 (V12) ---
            # 💡 結合(Merge)する列に V12新特徴量を追加
            merge_cols = ['Race_ID', 'Stage1_Rough_Prob', 'F_Program_Distortion_Index', 'F_Dash_ST_Threat']
            ds2 = df_s2[df_s2['Project_ID'] == pid].merge(ds1[merge_cols], on='Race_ID', how='inner')
            ...
            m1_1st_path = f"Models_Stage2_V12/LGBM_V12_Model1_1st_{pid}.pkl"
            m2_2nd_path = f"Models_Stage2_V12/LGBM_V12_Model2_2nd_{pid}.pkl"
            
            if not (os.path.exists(m1_1st_path) and os.path.exists(m2_2nd_path)): 
                logger.error(f"⚠️ {pid} の勝負レースがありますが、V12モデルが見つからないため推論をスキップします。")
                continue
    ...
    # 📊 判定プロセスの詳細コンソール出力
    logger.info("📊 === V12 AI推論 1レースごとの判定レポート ===")
