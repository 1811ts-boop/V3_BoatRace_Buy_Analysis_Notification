...
# 1. 環境設定・定数
# =============================================================================
logger = logging.getLogger("V7_DailyRun_2Fuku")
...
def prepare_ai_models(service):
    logger.info("🤖 2連複専用AIモデル（.pkl）をダウンロードします...")
...
def transform_for_inference_v7(df_raw, df_tide):
    fs1, fs2 = [], []
    error_count = 0 
...
def run_ai_and_notify_v7(df_s1, df_s2):
    ...
    for pid in PROJECT_IDS:
        ...
        try:
            ...
            for rid, grp in ds2.groupby('Race_ID', sort=False):
                ...
                # 💡 2連複の確率クロス計算 (15通り)
                sc = {}
                for combo in itertools.combinations([1, 2, 3, 4, 5, 6], 2):
                    b1, b2 = combo
    ...
    # 📊 判定プロセスの詳細コンソール出力
    logger.info("📊 === AI推論 1レースごとの判定レポート (2連複版) ===")
    ...
    if not buys:
        msg = f"🤖 【V7 2連複・真・聖杯AI】\n📅 {TODAY_OBJ.strftime('%Y年%m月%d日')}\n本日は「新・聖杯カレンダー」に合致するレースがありませんでした🙅‍♂️"
...
def main():
    logger.info("V7 System Start (2Fuku Robust Edition)")
    ...
    s1, s2 = transform_for_inference_v7(df, dtide)
    if not s1.empty and not s2.empty:
        run_ai_and_notify_v7(s1, s2)
