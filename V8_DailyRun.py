...
# 1. 環境設定・定数
# =============================================================================
logger = logging.getLogger("V8_DailyRun_2Tan_Sniper")
...
PORTFOLIO_CSV_NAME = "V8_Combined_Portfolio_Calendar.csv" 
...
def prepare_ai_models(service):
    logger.info("🤖 V8 AIモデル＆ポートフォリオ設定ファイルをダウンロードします...")
    ...
    download_latest_file_by_name(service, PORTFOLIO_CSV_NAME)
    
    for pid in PROJECT_IDS:
        download_latest_file_by_name(service, f"LGBM_Stage1_2Fuku_{pid}.pkl", "Models_Stage1_2Fuku")
        download_latest_file_by_name(service, f"LGBM_Stage2_1st_2Fuku_{pid}.pkl", "Models_Stage2_2Fuku")
        download_latest_file_by_name(service, f"LGBM_Stage2_2nd_2Fuku_{pid}.pkl", "Models_Stage2_2Fuku")
...
def transform_for_inference(df_raw, df_tide):
    fs1, fs2 = [], []
    error_count = 0 
...
def run_v8_inference_and_notify(df_s1, df_s2):
    ...
    # 💡 修正①: Stage 1の変換辞書に `PlaceID` を追加！これがないと場が1つズレて誤認されます
    CATEGORIES_DEF_S1 = {'PlaceID': list(range(1, 25)), 'Course_Type': [1, 2, 3, 4, 5], 'Weather_Code': [1, 2, 3], 'Tide_Trend': [-1, 0, 1]}
    ...
    for pid in PROJECT_IDS:
        ...
        logger.info(f"🧠 {pid} の推論・照合を開始...")
        
        try:
            # --- Stage 1: 荒れ度予測 ---
            with open(f"Models_Stage1_2Fuku/LGBM_Stage1_2Fuku_{pid}.pkl", 'rb') as f: m1 = pickle.load(f)
            ...
            # --- Stage 2: 2連単確率予測 ---
            ds2 = df_s2[df_s2['Project_ID_Calc'] == pid].merge(ds1[['Race_ID', 'Stage1_Rough_Prob', 'Scheduled_Time']], on='Race_ID', how='inner')
            with open(f"Models_Stage2_2Fuku/LGBM_Stage2_1st_2Fuku_{pid}.pkl", 'rb') as f: m2_1 = pickle.load(f)
            with open(f"Models_Stage2_2Fuku/LGBM_Stage2_2nd_2Fuku_{pid}.pkl", 'rb') as f: m2_2 = pickle.load(f)
            
            X2 = ds2[m2_1.feature_name()].copy()
            ...
            ds2['P1'], ds2['P2'] = m2_1.predict(X2_array), m2_2.predict(X2_array)
            ...
            for rid, grp in ds2.groupby('Race_ID', sort=False):
                ...
                time_str = grp['Scheduled_Time'].iloc[0]
                ...
                sc = {}
                for combo in itertools.permutations([1, 2, 3, 4, 5, 6], 2):
                    b1, b2 = combo
                    sc[f"{b1}-{b2}"] = p1[b1] * p2[b2]
                    
                best_ticket = sorted(sc.items(), key=lambda x: x[1], reverse=True)[0][0]
                
                buys.append({
                    'p': plid, 'r': rnum, 'time': time_str, 'pid': pid, 
                    'c': cat, 'strat': matched_strategy, 'ticket': best_ticket
                })
                
        except Exception as e: 
            logger.error(f"AI Error ({pid}): {e}")

    logger.info("📊 === AI推論 1レースごとの判定レポート (V8 2連単版) ===")
    ...
    if not buys:
        msg = f"🤖 【V8 2連単・全天候型AI】\n📅 {TODAY_OBJ.strftime('%Y年%m月%d日')}\n本日は「全天候型ポートフォリオカレンダー」に合致するレースがありませんでした🙅‍♂️\n（無駄撃ちせず、資金を温存します）"
        logger.info("本日は条件合致レースがありませんでした。LINEに通知します。")
        send_line_broadcast(msg)
    else:
        buys = sorted(buys, key=lambda x: x['time']) 
        msg = f"🤖 【V8 2連単・全天候型AI】\n📅 {TODAY_OBJ.strftime('%Y年%m月%d日')}\n✅ 合致：{len(buys)}レース（2連単1点買い）\n"
        
        for b in buys:
            place_name = JCD_MAP.get(f"{b['p']:02d}", "不明")
            grade = "SG" if b['pid'] == "P0_SG" else "G1" if b['pid'] == "P1_G1_Elite" else "一般" if b['pid'] == "P3_General_Std" else "企画"
...
def main():
    logger.info("V8 System Start (2Tan All-Weather Edition)")
    ...
    # 💡 修正②: 呼び出し関数名を正しく直しました
    s1, s2 = transform_for_inference(df, dtide)
    if not s1.empty and not s2.empty:
        run_v8_inference_and_notify(s1, s2)
